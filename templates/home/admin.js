...
mediaUploader = function(a, m){
    switch(a){
        ...
        case 'complete':
            ...
            m.thumb = m.thumb ? m.thumb : '/themes/'+THEME+'/images/file.png';
            ...
            .load(function(){
                ...
                $(this).bind('click',function(){ mediaInsert(m.type,'/'+m.thumb,m.alt,'/'+m.src); });
            });
        break;
    }
};
var mediaAdd = function(type, thumb, alt, src){
    ...
    var content = '<a href="'+src+'" class="file" title="'+alt+'">'+alt+'</a>';
    //editor.selection.insertContent(content);
    $('#pagecontent').wysiwyg('insertHtml', content);
    $('#result').empty();
};
var mediaInsert = function(type, thumb, alt, src){
    var content = '<div class="mediaPop">'
        ...
        +'<a href="javascript:mediaAdd(\''+type+'\',\''+thumb+'\',\''+alt+'\',\''+src+'\');" class="button right">'+lang.add+'</a>'
        +'<div class="clear"></div></div>';
    $(document).keydown(function(e){
        if(e.keyCode == 32){
            mediaAdd(type, thumb,alt,src);
            $('#result').empty();
