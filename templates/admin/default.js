...
var tool = {
    ...
    post: function(a, form){
        $.ajax({
            url: form ? form.attr('action') : a.url,
            type: 'post',
            data: (form ? form.serialize() : a.data) + '&js=1',
            dataType: a.type ? a.type : 'json',
            success: a.onComplete|| function(){}
        });
    },
    form: function (custom,initiation,complete){
        //custom form element
        if(custom){
            $('a.checkboxItem').each(function(){
                $(this).bind({
                    'keydown':function(e){
                        if(e.keyCode == 32) tool.checkbox($(this));
                    },
                    'click':function(){
                        tool.checkbox($(this));
                    }
                });
                if($('#'+$(this).attr('title')).val() > 0)
        ...
        $('#form').bind('submit', function(e) {
            ...
            tool.post({
                'onComplete':function(r){
                    if(r){
                        if(r.code == 200 && complete) _complete(r.message);
    ...
    linkposts: function (){
        $('a.linkpost').each(function(){
            var link =  $(this).attr('href').split('?');
            $(this).attr('href','javascript:tool.empty();');
            //$(el).set('href','javascript:tool.linkpost(\''+el+'\',\''+link[0]+'\',\''+link[1]+'\')');
            $(this).bind({
                //'keydown':function(event){ if(event.key == 'space') tool.checkbox(e); },
                'click':function(){
                    tool.linkpost($(this),link[0],link[1]);
                }
            });
        });
    },
    linkpost: function (el, link, data){
        el.attr('disabled','disabled');
        $('#result').empty();
        el.addClass('loader');
        var func = 'tool.post({\'url\':\''+link+'\',\'data\':\''+data+'\',\'onComplete\':function(result){if(result)tool.result(result.code,result.status,result.message,result.fields,result.redirect,result.debug)}});';
        tool.confirm('header', el.attr('title'), func);
    },
    valuelabels: function (){
        $('input.valuelabel').each(function(){
            $(this).attr('data-label',$(this).val());
            $(this).bind({
                'focus':function(){
                    tool.valuelabel($(this));
                },
                'blur':function(){
                    tool.valuelabel($(this));
                }
    ...
    valuelabel: function (el){
        if(el.val() && el.val() == el.attr('data-label')){
            el.val('').removeClass('valuelabel');
        } else if(!el.val()) {
            el.val(el.attr('data-label')).addClass('valuelabel');
        }
    },
    marker: function(el){
        $(el).focus();
    ...
    pop: function(content){
        ...
        //console.log(((window.innerHeight - $('result').getSize().y)*.5 + window.pageYOffset)+'px', window.innerHeight, window.pageYOffset);
        //$('result').css('top',((window.innerHeight - $('result').getSize().y)*.5 + window.pageYOffset));
        return 0;
    },
    result: function(code, status, message, fields, redirect, debug){
        ...
        /*if(fields) fields.each(function(el){
            $('#'+el).focus();
        ...
        return 0;
    //if(code < 600) setTimeout('if($(\'result\').getFirst())$(\'result\').getFirst().fade(0);',2000);
    },
    confirm: function(header, msg, func){
        ...
        this.pop(content);
        $('#result').removeAttr('class').addClass('alert');
    },
    sortable: function(ID, handle, a){
        $(function() {
            $('#'+ID).sortable({
                handle: handle || 'img',
    ...
    inlineedit: function(el){
        var eventfunc = function(el, input, e){
            if((!e || e.keyCode == 13) && input.data('html') != input.val()) { // enter
                if(e) e.preventDefault();
                tool.post({
                    'url':input.data('url'),
                    'data':'ID='+input.data('id')+'&value='+input.val(),
                    'onComplete':function(r){
                        el.html(r.message.value).show();
                        input.remove();
                    }
                });
            } else if(!e || e.keyCode == 13 || e.keyCode == 27){ //esc
                if(e) e.preventDefault();
                el.html(input.data('html')).show();
                input.remove();
            }
        };
        el.click(function(){
            ...
            .bind({
                'keydown':function(e){
                    return eventfunc(el, input, e);
                },
                'blur':function(){
                    return eventfunc(el, input);
                }
            });
            el.after(input.css({
                'width':(el.css('width') | el.width()),
                'height':(el.css('height') | el.height()),
                ...
                'padding-bottom':el.css('padding-left'),
                'padding-left':el.css('padding-bottom'),
    ...
    inlineedits: function(){
        $('.inlineedit').each(function(){
            tool.inlineedit($(this));
        });
    },
    spammasks: function(){
    //var pattern = /(([_.0-9a-z-]+)[ ]?\[at][ ]?(([0-9a-z][0-9a-z-]+.)\.[a-z]{2,4}))/g;
    //$('page').set('html',$('page').get('html').replace(pattern, '<a href="mailto:$2@$3">$2@$3</a>'));
    },
    checkbox: function(item){
        if($('#'+item.attr('title')).val() > 0){
            $('#'+item.attr('title')).val(0);
            item.removeClass('checked');
        } else {
            $('#'+item.attr('title')).val(1);
            item.addClass('checked');
        }
    },
    lengthcheck: function(ID, min, max){
        var c = $('#'+ID).val().length;
        if(!$('#'+ID+'-count').length)
            $('<span>',{
                'id':ID+'-count',
