    ...
    var m = {
        init: function(form){
            ...
            .each(function(){
                ...
                //console.log(type);
                type = type == 'checkbox' ? ( $(this).hasClass('switch') ? 'switch' : 'checkbox') : type;
                $('<p class="'+ type +'">')
        ...
        error: function(form,r){
            if(r.fields){
                var f = r.fields.split(/,/);
                for(var i in f){
                    form.find('[name='+f[i]+']').addClass('error');
                     console.log('[name='+f[i]+']');
                }
            }
            form.find('label:last-child')
                .before($('<i class="error">').html(r.message));
        },
        complete: function(form,r){
            form.find('input[type=submit]').removeAttr('disabled');
            form.find('.loader').css('display','none');
            if(r.code != 200) m.error(form, r);
